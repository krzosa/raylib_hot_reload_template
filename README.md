...
1. You need either the gcc compiler (which is packaged with raylib installation) or the msvc compiler (which you can find how to setup here https://hero.handmade.network/episode/code/day001/) the compiler needs to be on your system path so that the system can find it.
2. After you get the compiler, run the build-raylib script of your choice, which is going to create a dynamic library of raylib, that library is going to get copied into the bin folder. 
3. Call the build script of your choice.
4. Call the run script or execute the binary manually.

## Structure

* win32_main.c -> Loads the dynamic library and calls Update from the dynamic library in a loop. On every iteration checks if the dynamic library changed, if it did then it swaps the code 
